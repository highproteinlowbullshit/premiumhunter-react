...
const FAQ_ITEMS = [
  {
    ...
    a: 'Premium Hunter has a free tier and a Pro tier. Paper trading, the watchlist, and basic portfolio tracking are free. Pro unlocks the IV rank screener, Greeks dashboard, advanced portfolio analytics, the AI Market Pulse briefing, and additional screener features. A 14-day free trial is available on Pro — no credit card required to start.',
  },
  {
...
const FEATURES = [
  {
    ...
    body: 'Scan 488 actively traded stocks and ETFs ranked by Implied Volatility rank. Instantly identify which stocks have elevated IV — the ideal conditions for selling premium. Filter by sector, price range, and IV threshold to find your next trade in seconds.',
  },
  {
    icon: (
      <svg width="22" height="22" viewBox="0 0 22 22" fill="none">
  ...
  },
  {
    icon: (
      <svg width="22" height="22" viewBox="0 0 22 22" fill="none">
        <path d="M3 5h16M3 11h10M3 17h7" stroke="#00e5c4" strokeWidth="1.4" strokeLinecap="round" />
        <circle cx="18" cy="15" r="3.5" stroke="#00e5c4" strokeWidth="1.2" />
        <path d="M16.5 15h1M18 13.5v1" stroke="#00e5c4" strokeWidth="1" strokeLinecap="round" />
      </svg>
    ),
    title: 'AI Market Pulse',
    body: 'A twice-daily AI briefing on overall market conditions — generated before the open and after the close. Sentiment score, IV environment rating, sector rotation signals, earnings alerts, and curated news — so you start every session informed.',
  },
];
...
const FEATURE_DETAILS = [
  ...
  },
  {
    ...
    tagColor: '#f87171',
    title: 'AI Market Pulse',
    tagline: 'Twice-daily AI briefing on market conditions — pre-market and post-market.',
    icon: (
      <svg width="20" height="20" viewBox="0 0 22 22" fill="none">
        <path d="M3 5h16M3 11h10M3 17h7" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round"/>
        <circle cx="18" cy="15" r="3.5" stroke="currentColor" strokeWidth="1.2"/>
        <path d="M16.5 15h1M18 13.5v1" stroke="currentColor" strokeWidth="1" strokeLinecap="round"/>
      </svg>
    ),
    forBeginners: 'Every morning before the market opens and every evening after it closes, Premium Hunter publishes an AI-generated Market Pulse. Think of it as a short briefing from a well-read analyst who has scanned dozens of financial news articles overnight and summarised what matters for wheel traders. You get a single sentiment score — bullish, neutral, or bearish — plus a plain-English summary of what is driving the market that day. If there are earnings announcements coming up that week for stocks you might trade, they are flagged. If volatility is elevated and conditions are good for selling premium, the IV environment score tells you that directly. For beginners, this replaces the daily habit of reading three different financial sites before deciding whether to open a new position.',
    forExperienced: 'The Market Pulse is generated by an LLM trained on real financial news, running twice per day: at 8:00 AM ET (pre-market) and 5:30 PM ET (post-market). Each run fetches and scores the most relevant market news from Finnhub and Yahoo Finance RSS, deduplicates articles, and passes the top 20 most relevant pieces into the model. Output fields include: sentiment score (0–100), market sentiment label, headline summary, options context paragraph, key themes list, VIX context, IV environment score (0–100 — above 60 is favourable for premium sellers), sector rotation pulse (bullish/neutral/bearish per sector with a note), earnings this week (any stocks expiring within 7 days with days-until and timing), live market context (SPY %, VIX level, BTC %), and a per-ticker context map used for position impact matching. Position impact matching is client-side: it checks your open positions against the mentioned_ticker_context field and shows relevant news context inline — zero AI calls per user.',
    bullets: [
      'Runs twice daily: pre-market (8:00 AM ET) and post-market (5:30 PM ET)',
      'Sentiment score 0–100 with label: bullish / slightly bullish / neutral / slightly bearish / bearish',
      'IV environment score: 0–100 rating for premium-selling conditions, derived from live IV rank data',
      'Sector pulse: bullish / neutral / bearish signal per sector with a one-line note',
      'Earnings this week: any tickers with upcoming earnings, flagged by days-until and timing (pre/post)',
      'Live market strip: SPY %, VIX level, BTC % from the time of generation',
      'Curated news tab: source articles ranked by relevance with source, headline, and related tickers',
      'Tickers tab: mentioned tickers with a brief AI-written context note for each',
      'Position impact: your open wheel positions matched against mentioned tickers — shows relevant context inline',
      'Sentiment sparkline: 10-session trend of market sentiment score so you can see regime shifts at a glance',
    ],
  },
  {
    id: 'checklist',
